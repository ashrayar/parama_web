...
pdb_id=sys.argv[1]
logfile.write(pdb_id+"\n")
chain_id=sys.argv[2]
logfile.write(chain_id+"\n")
wanted_residues_str=sys.argv[3].split(",")
present_in_db=sys.argv[4]
logfile.write("in pep unit program\n")
logfile.write(present_in_db+"\n")
logfile.close()
wanted_residues=[int(x) for x in wanted_residues_str]
...
if present_in_db=="True":
    ...
    if not atleast_one_map:
        ...
        htmlfile.close()
logfile=open("rotation_log.log","a") 
logfile.write("finished getting results from db\n")
logfile.close()
msg=MIMEMultipart()
toaddr=sys.argv[5]
